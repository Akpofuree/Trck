...
export default function Page() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-black text-white">
      <aside className={`${sidebarOpen ? "w-[260px]" : "w-16"} hidden shrink-0 flex-col border-r border-white/10 bg-black transition-all duration-300 md:flex`}>
        <div className="flex h-[57px] items-center justify-between border-b border-gray-200 bg-white px-4 text-black">
          {sidebarOpen ? <Link href="/" className="text-lg font-extrabold tracking-wider">TRCK</Link> : null}
          <button
              ...
              {items.map((item) => (
                <Link
                  ...
                  className={`mb-1 flex items-center gap-3 rounded-[11.58px] px-3 py-2.5 font-montserrat text-[1.1rem] font-semibold ${
                    item.active ? "bg-[#ED5E2E] text-white" : "text-white/60 hover:bg-white/10 hover:text-white"
                  }`}
                >
                  <item.icon size={20} />
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="mt-0.5 text-[0.641rem] leading-none">Super Admin</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto bg-black p-4 md:p-6">
          <div className="mx-auto max-w-[80rem]">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-[2rem] font-bold text-white">Event Management</h2>
              <div className="flex gap-3">
                <button className="rounded-[0.35rem] border border-[#ff7a00] px-4 py-2 text-[0.85rem] text-[#ff7a00]">+ Create Event Series</button>
                <button className="rounded-[0.35rem] bg-[#ff7a00] px-4 py-2 text-[0.85rem] font-semibold text-white">+ Create Event</button>
              </div>
            </div>

            <div className="grid gap-4 lg:grid-cols-4">
              {["Total Events", "Upcoming Events", "Live Now", "Past Events"].map((label, i) => (
                <div key={label} className="rounded-2xl border border-white/10 bg-[#111115] p-6">
                  <p className="text-sm text-white/65">{label}</p>
                  <p className="mt-3 text-3xl font-bold text-[#ff7a00]">{[24, 12, 1, 8][i]}</p>
                </div>
              ))}
            </div>

            <div className="mt-4 grid gap-4 lg:grid-cols-4">
              {["Total Revenue", "This Month Revenue", "Avg Attendance", "Total Attendees"].map((label, i) => (
                <div key={label} className="rounded-2xl border border-white/10 bg-[#111115] p-6">
                  <p className="text-sm text-white/65">{label}</p>
                  <p className="mt-3 text-3xl font-bold text-[#ff7a00]">{["₦22.7M", "₦2780K", "70%", "6,035"][i]}</p>
                </div>
              ))}
            </div>
            ...
            <div className="mt-6 rounded-2xl border border-white/10 bg-[#111115] p-6">
              <div className="flex items-center gap-3">
                <button className="rounded bg-[#ff7a00] px-3 py-1 text-sm text-white">List</button>
                <button className="rounded bg-white/5 px-3 py-1 text-sm text-white/60">Calendar</button>
                <button className="rounded bg-white/5 px-3 py-1 text-sm text-white/60">Timeline</button>
                <div className="ml-auto flex gap-3">
                  <div className="rounded-full border border-white/10 px-4 py-2 text-sm text-white/45">Search by event name, date or location</div>
                  <button className="rounded bg-[#ff7a00] px-4 py-2 text-sm text-white">Filters</button>
                </div>
              </div>
                  ...
                  <div key={n} className="rounded-2xl bg-[#171717] p-4">
                    <div className="flex items-start gap-4">
                      <div className="relative h-16 w-16 overflow-hidden rounded">
                        ...
                        <span className="absolute left-1 top-1 rounded bg-[#26e65d] px-1.5 py-0.5 text-[0.55rem] font-semibold text-black">LIVE</span>
                      </div>
                      <div className="flex-1">
                        <div className="flex items-start justify-between gap-3">
                          <h3 className="text-lg font-semibold text-white">5IVE LIVE AT O2 ARENA</h3>
                        ...
                        <p className="mt-1 text-sm text-white/55">Concert · Part of Davido live at O2</p>
                        <p className="mt-1 text-sm text-white/55">Friday, Nov 15, 2024 · O2 Arena, London, United Kingdom</p>
                        <p className="mt-2 text-sm text-white/55">Sold: 156/200 (78%) · Revenue: ₦450,000 · Check-ins: 145/156</p>
                      </div>
                    </div>
                  </div>
                ))}
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="mt-3 flex items-center gap-3">
                          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-white/90 text-black text-xs font-semibold">E</div>
                          <div className="text-xs text-white/75">
                            Hosted by Elizabeth R Events
                          </div>
                        </div>
                      </div>
                      <button className="rounded-[0.45rem] bg-[#ff7a00] px-4 py-2 text-sm font-semibold text-white">View Event →</button>
                    </div>
                  </div>
                </div>
                <div className="px-4 py-10 text-center">
                  <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-[#26e65d] text-[1.4rem] font-bold text-black">✓</div>
                  <h4 className="text-[2.25rem] font-semibold text-white">Published!</h4>
                  <p className="mx-auto mt-3 max-w-md text-[0.75rem] leading-snug text-white/55">
                    Congratulations! you have successfully Published your event and can now manage engagement.
                  </p>
                </div>
              </div>
            </div>
