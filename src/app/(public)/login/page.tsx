"use client";

import Image from "next/image";
import Link from "next/link";
import { Logo } from "@/components/shared/logo";
import { useState } from "react";
import { Eye, EyeOff, ChevronDown } from "lucide-react";

export default function Page() {
  const [showPassword, setShowPassword] = useState(false);
  const [remember, setRemember] = useState(false);

  return (
    <div className="flex min-h-screen font-[var(--font-inter)]">
      {/* ── Left: Form Panel ── */}
      <div className="relative flex w-full flex-col justify-between bg-white px-8 py-10 lg:w-[55%] xl:w-[50%]">
        {/* Logo */}
        <div>
          <Logo width={120} height={44} className="h-8 w-auto" />
        </div>

        {/* Form */}
        <div className="mx-auto w-full max-w-[420px]">
          <p className="mb-7 text-[0.95rem] text-gray-600">
            Enter the email address associated with your Trck account
          </p>

          <div className="space-y-4">
            {/* Email */}
            <div>
              <input
                id="login-email"
                type="email"
                placeholder="Email Address"
                className="w-full rounded-[0.5rem] border border-gray-200 bg-gray-50 px-4 py-3.5 text-[0.9rem] text-gray-800 outline-none transition-all placeholder:text-gray-400 focus:border-[#ED5A2E] focus:ring-2 focus:ring-[#ED5A2E]/20"
              />
            </div>

            {/* Password */}
            <div className="relative">
              <input
                id="login-password"
                type={showPassword ? "text" : "password"}
                placeholder="Password"
                className="w-full rounded-[0.5rem] border border-gray-200 bg-gray-50 px-4 py-3.5 pr-12 text-[0.9rem] text-gray-800 outline-none transition-all placeholder:text-gray-400 focus:border-[#ED5A2E] focus:ring-2 focus:ring-[#ED5A2E]/20"
              />
              <button
                id="toggle-password"
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 transition-colors hover:text-gray-600"
              >
                {showPassword ? (
                  <EyeOff className="h-4 w-4" />
                ) : (
                  <Eye className="h-4 w-4" />
                )}
              </button>
            </div>

            {/* Remember + Forgot */}
            <div className="flex items-center justify-between">
              <label
                htmlFor="remember-me"
                className="flex cursor-pointer items-center gap-2 text-[0.85rem] text-gray-600"
              >
                <input
                  id="remember-me"
                  type="checkbox"
                  checked={remember}
                  onChange={() => setRemember(!remember)}
                  className="h-3.5 w-3.5 cursor-pointer accent-[#ED5A2E]"
                />
                Remember me
              </label>
              <Link
                href="/forgot-password"
                className="text-[0.85rem] text-gray-600 hover:text-[#ED5A2E] transition-colors"
              >
                Forgot password?
              </Link>
            </div>

            {/* Login Button */}
            <button
              id="login-submit"
              type="submit"
              className="w-full rounded-full bg-[#ED5A2E] py-3.5 text-[0.95rem] font-semibold text-white shadow-md shadow-[#ED5A2E]/30 transition-all hover:bg-[#d4501f] hover:shadow-lg hover:shadow-[#ED5A2E]/40 active:scale-[0.98]"
            >
              Login
            </button>
          </div>

          {/* Divider */}
          <div className="my-6 flex items-center gap-4">
            <div className="h-px flex-1 bg-gray-200" />
            <span className="text-[0.85rem] text-gray-400">or</span>
            <div className="h-px flex-1 bg-gray-200" />
          </div>

          <p className="mb-4 text-center text-[0.85rem] text-gray-500">
            Login with Social Media Platforms
          </p>

          {/* Social Icons */}
          <div className="flex items-center justify-center gap-5">
            {/* Facebook */}
            <button
              id="login-facebook"
              className="flex h-11 w-11 items-center justify-center rounded-full border border-gray-200 bg-white shadow-sm transition-all hover:scale-110 hover:shadow-md"
            >
              <svg viewBox="0 0 24 24" className="h-5 w-5 fill-[#1877F2]">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
              </svg>
            </button>

            {/* Google */}
            <button
              id="login-google"
              className="flex h-11 w-11 items-center justify-center rounded-full border border-gray-200 bg-white shadow-sm transition-all hover:scale-110 hover:shadow-md"
            >
              <svg viewBox="0 0 24 24" className="h-5 w-5">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
              </svg>
            </button>

            {/* Apple */}
            <button
              id="login-apple"
              className="flex h-11 w-11 items-center justify-center rounded-full border border-gray-200 bg-white shadow-sm transition-all hover:scale-110 hover:shadow-md"
            >
              <svg viewBox="0 0 24 24" className="h-5 w-5 fill-black">
                <path d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.546 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.182-1.221-3.22-4.857-.026-3.04 2.48-4.494 2.597-4.559-1.429-2.09-3.623-2.324-4.39-2.376-2-.156-3.675 1.09-4.61 1.09zM15.53 3.83c.843-1.012 1.4-2.427 1.245-3.83-1.207.052-2.662.805-3.532 1.818-.78.896-1.454 2.338-1.273 3.714 1.338.104 2.715-.688 3.559-1.701" />
              </svg>
            </button>
          </div>

          <p className="mt-6 text-center text-[0.875rem] text-gray-500">
            Don&apos;t have an account?{" "}
            <Link
              href="/signup"
              className="font-semibold text-[#ED5A2E] hover:underline"
            >
              Signup
            </Link>
          </p>
        </div>

        {/* Footer */}
        <div className="flex items-center justify-between text-[0.78rem] text-gray-400">
          <button id="language-picker" className="flex items-center gap-1 hover:text-gray-600 transition-colors">
            English (United Kingdom)
            <ChevronDown className="h-3.5 w-3.5" />
          </button>
          <Link href="/privacy" className="hover:text-gray-600 transition-colors">
            Privacy Policy
          </Link>
        </div>
      </div>

      {/* ── Right: Background + QR Panel ── */}
      <div className="relative hidden flex-col items-center justify-center lg:flex lg:w-[45%] xl:w-[50%]">
        {/* Background image */}
        <Image
          src="/carnival-hero-bg.jpg"
          alt="Trck carnival experience"
          fill
          className="object-cover"
          priority
        />
        {/* Dark overlay */}
        <div className="absolute inset-0 bg-black/45" />

        {/* QR Card */}
        <div className="relative z-10 flex flex-col items-center rounded-2xl bg-white/10 px-10 py-8 text-center backdrop-blur-md border border-white/20 shadow-2xl">
          {/* QR code grid simulation */}
          <div className="grid h-[130px] w-[130px] grid-cols-7 gap-[2px] rounded-md overflow-hidden">
            {Array.from({ length: 49 }).map((_, i) => {
              const corners = [0,1,2,3,4,5,6,7,13,14,20,21,27,28,34,35,41,42,43,44,45,46,47,48];
              const isCorner = corners.includes(i);
              const isDark = isCorner || Math.random() > 0.55;
              return (
                <div
                  key={i}
                  className={`rounded-[1px] ${isDark ? "bg-white" : "bg-white/10"}`}
                />
              );
            })}
          </div>

          <p className="mt-5 text-[0.95rem] font-semibold text-white">
            Login with QR code
          </p>
          <p className="mt-1.5 max-w-[180px] text-[0.78rem] text-white/70">
            Scan this code with your phone camera to log in instantly
          </p>
        </div>
      </div>
    </div>
  );
}
