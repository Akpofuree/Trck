  ...
  return (
    <div className="border-b border-[#ED5A2E]/40 bg-[#151515] px-6 sm:px-8 py-4">
      <div className="mx-auto flex max-w-[1440px] items-center gap-4">
        <Link href="/checkout" className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20 transition-all">
...
function SectionCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="rounded-[20px] bg-[#121212] border border-white/10 p-5 sm:p-6 shadow-md">
      {title ? <h3 className="text-sm sm:text-base font-bold text-white mb-4">{title}</h3> : null}
      <div>{children}</div>
    </section>
  );
}
              ...
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-xl bg-[#1A1A1A] p-4 border border-white/5">
                  <div className="space-y-1">
                    ...
                    <h4 className="text-base font-bold text-white mt-1">5IVE LIVE AT 02 ARENA</h4>
                    <p className="text-xs text-white/60">Nov 15, 2024 • 7:00 PM • 02 Arena London</p>
                  </div>
            ...
            <SectionCard title="Important Information &amp; Policies">
              <div className="space-y-3.5 text-xs text-white/70">
                <div>
                  ...
                  <p className="mt-0.5 leading-relaxed text-white/60">Please arrive 30 minutes before the event start time. Late arrivals may not be admitted until a suitable break.</p>
                </div>
                <div>
                  <p className="font-bold text-white">Refund Policy</p>
                  <p className="mt-0.5 leading-relaxed text-white/60">Full refund available up to 24 hours before the event. No refunds after this period.</p>
                </div>
                <div>
                  <p className="font-bold text-white">Age Restrictions</p>
                  <p className="mt-0.5 leading-relaxed text-white/60">This event is 18+ only. Valid government-issued ID required at entry. No exceptions.</p>
                </div>
            ...
            <SectionCard title="What Our Customers Say">
              <div className="space-y-4">
                {Array.from({ length: 2 }).map((_, idx) => (
                  <div key={idx} className="flex items-start gap-3.5 border-b border-white/5 pb-4 last:border-b-0">
                    <div className="h-10 w-10 rounded-full bg-[#b76d4c] shrink-0" />
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-xs font-bold text-white">Jane Doe</p>
                          <p className="text-[11px] text-white/45">November 3rd, 2025</p>
                        </div>
